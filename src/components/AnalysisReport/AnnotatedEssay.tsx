...
const AnnotatedEssay: React.FC<Props> = ({ data }) => {
  ...
  const [segments, setSegments] = useState<
    (string | { text: string; annotation: Annotation })[]
  >([]);
  ...
  useEffect(() => {
    ...
    const addAnnotation = (
      item: DetailedError | Optimization,
      type: "error" | "optimization",
    ) => {
                ...
                <div className="flex-1 min-w-0 space-y-3">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      <span
                        className={`text-[10px] font-bold uppercase tracking-wider ${isError ? "text-red-500" : "text-blue-500"}`}
                      >
                        {isError
                          ? (detail as DetailedError).type
                          : "Expressive Upgrade"}
                      </span>
                    </div>
